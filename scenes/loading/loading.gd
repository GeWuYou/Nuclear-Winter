...
func load_game(slot_name: String) -> void:
	var error = Dialogic.Save.load(slot_name)
	if error == OK:
		print("从存档槽位 %s 加载成功" % slot_name)
	else:
		print("从存档槽位 %s 加载失败，错误码: %d" % [slot_name, error])
...
func _on_load_pressed() -> void:
	if not current_slot_name == 'slot_null':
		...
		if current_scene_path == "res://scenes/main_scene/main_scene.tscn":
			loading.hide()
		else:
